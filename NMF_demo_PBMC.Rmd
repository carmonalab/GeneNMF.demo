...
if (!file.exists(data.path)) {
    dir.create(ddir)
    dataUrl <- "https://ndownloader.figshare.com/files/60037946"
...
# Intepretation of gene programs by overrepresentation analysis
...
For example, MP4 appears to overlap significantly with natural killer cell (NK) signatures:
```{r}
head(top_p$MP4)
```
...
b <- DimPlot(seu, reduction = "umap_MP", group.by = "donor") + theme(aspect.ratio = 1,
                                                            axis.text = element_blank(),
    axis.title = element_blank(),
    axis.ticks = element_blank()) + ggtitle("Donor")
a | b
```
