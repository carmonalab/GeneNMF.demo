...
nmf.res <- run_NMF(seu.list, assay="RNA", slot="data", k=4:9, L1=c(0,0),
                    do_centering=TRUE, use_var_feat=TRUE, nfeatures = 2000,
...
ph <- get_NMF_heatmap(nmf.res, method=0.5, max.genes=50,
...
nprograms = 10
nmf.genes <- get_NMF_genes_consensus(nmf.res, method=0.5, max.genes=50,
...
```{r}
nmf.cov <- get_NMF_genes_consensus(nmf.res, method=0.5, max.genes=50,
                                    hclust.method="ward.D2", nprograms=nprograms,
...
top_p <- lapply(nmf.genes, function(program) {
  run_GSEA(program, universe=rownames(seu), category = "C8")
})
...
```{r}
ndim <- 15
seu <- RunNMF(seu, k = ndim, nfeatures = 2000, L1=c(0.05,0), use_var_feat = T)
Reductions(seu)
...
```{r message=F, results=F, warning=F}
nmf.res <- run_NMF(seu.list, assay="RNA", slot="data", k=4:9, L1=c(0,0),
                    do_centering=TRUE, use_var_feat=TRUE, nfeatures = 1000,
                    exclude_ribo_mito=FALSE)
```
